  ...
  <div class="min-h-screen bg-white text-slate-800">
    ...
    <div v-else-if="fetchError" class="min-h-[50vh] flex flex-col items-center justify-center px-4">
      <div class="bg-red-50 border border-red-200 rounded-xl p-8 max-w-md text-center">
        ...
        <button @click="$router.push('/catalog')" class="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition-colors">
          Kembali ke Katalog
        </button>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 items-start">
        ...
        <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden flex flex-col h-full relative">
           ...
           <div class="relative w-full aspect-[5/4] bg-gradient-to-b from-slate-50 to-white flex items-center justify-center p-6 overflow-hidden">
             <img :src="product.image" :alt="product.title" class="w-full h-full object-contain mix-blend-multiply drop-shadow-lg" />
           </div>

           <div class="p-5 flex flex-col flex-grow text-left">
             <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{{ product.brand }}</div>
             <h3 class="text-lg font-bold text-slate-800 leading-tight mb-2">{{ product.title }}</h3>
             ...
             <div class="mt-auto pt-4 border-t border-dashed border-slate-200">
                ...
                <button @click="addToCart" class="w-full bg-[#D92525] hover:bg-red-700 text-white text-sm font-bold py-2.5 px-4 rounded-lg shadow-md flex items-center justify-center gap-2 transition-all active:scale-95">
                   <ShoppingBag class="w-4 h-4" />
                   <span>Beli Unit Baru</span>
                </button>
                <p class="text-[10px] text-center text-slate-400 mt-3">*Garansi resmi & support teknis.</p>
             </div>
           </div>
        </div>
             ...
             <table class="w-full text-sm">
              <tbody>
                <tr v-for="(val, key) in product.details.engine" :key="key" class="border-b border-gray-100 last:border-0 hover:bg-blue-50/20 transition-colors">
                  <td class="px-3 py-3 text-gray-500 font-medium whitespace-nowrap w-[40%] bg-gray-50/30 border-r border-gray-100 capitalize text-xs text-left">{{ formatKey(key) }}</td>
                  <td class="px-3 py-3 text-gray-800 font-semibold pl-4 text-xs text-left">{{ val }}</td>
                </tr>
              </tbody>
            </table>
          </div>
              ...
              <table class="w-full text-sm">
                <tbody>
                  <tr v-for="(val, key) in product.details.power" :key="key" class="border-b border-gray-100 last:border-0 hover:bg-blue-50/20">
                    <td class="px-3 py-3 text-gray-500 font-medium whitespace-nowrap w-[40%] bg-gray-50/30 capitalize border-r border-gray-100 text-xs text-left">{{ formatKey(key) }}</td>
                    <td class="px-3 py-3 text-gray-800 font-semibold pl-4 text-xs text-left">{{ val }}</td>
                  </tr>
                </tbody>
              </table>
        ...
        <div class="flex flex-col gap-6">
           <div class="flex flex-col shadow-sm rounded-sm overflow-hidden border border-gray-100">
            ...
            <div class="bg-white text-sm">
              <div class="bg-gray-50 px-3 py-2 font-bold text-gray-600 text-[10px] border-b border-gray-100 uppercase tracking-wider text-left">Open Type</div>
              ...
              <div class="bg-gray-50 px-3 py-2 font-bold text-gray-600 text-[10px] border-b border-gray-100 uppercase tracking-wider text-left">Silent Type</div>
              <div class="px-4 py-3">
                <div class="flex justify-between items-center mb-1">
                   <span class="text-gray-400 whitespace-nowrap text-xs">Size</span>
          ...
          <div class="bg-gradient-to-br from-orange-50 to-orange-100/50 rounded-lg border border-orange-200 p-5 relative overflow-hidden group">
            ...
            <div class="relative z-10 text-left">
               <div class="flex items-center gap-2 mb-2">
                 <div class="bg-orange-500 text-white p-1 rounded"><CalendarClock class="w-4 h-4" /></div>
               ...
               <p class="text-xs text-orange-900/80 font-medium leading-relaxed mb-4">
                 Butuh tipe iniuntuk proyek jangka pendek? Sinar Elektro juga menyediakan <strong>Sewa Genset</strong> (Menggunakan armada rental terpisah).
               </p>
               <button @click="openRentalModal" class="w-full bg-white border border-orange-300 text-orange-700 hover:bg-orange-600 hover:text-white hover:border-orange-600 font-bold py-2.5 px-4 rounded shadow-sm text-xs flex items-center justify-center gap-2 transition-all duration-300">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
       <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[90vh] flex flex-col">
         ...
         <div class="px-6 py-4 border-b flex justify-between items-center sticky top-0 z-10" :class="isBuyMode ? 'bg-red-50 border-red-100' : 'bg-orange-50 border-orange-100'">
            <div class="text-left">
            ...
            <button @click="closeModal" class="text-gray-400 hover:text-gray-700 transition-colors"><XCircle class="w-6 h-6" /></button>
         </div>
            ...
            <form @submit.prevent="isBuyMode ? submitBuyForm() : submitRentalForm()" class="flex flex-col gap-5">
              ...
              <div>
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" :value="product.title" readonly class="w-full border border-gray-200 bg-gray-100 text-gray-600 font-bold rounded px-3 py-2.5 text-sm pl-9 focus:outline-none cursor-not-allowed select-none shadow-inner">
                </div>
              ...
              <div class="grid grid-cols-2 gap-3 md:gap-4">
                  <div>
                    <label class="block text-sm font-normal text-gray-700 mb-1">Nama Lengkap <span class="text-red-500">*</span></label>
                    <input v-model="rentalForm.name" required type="text" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2" :class="isBuyMode ? 'focus:ring-red-500' : 'focus:ring-orange-500'" placeholder="Masukan nama lengkap">
                  </div>

                  <div>
                    <label class="block text-sm font-normal text-gray-700 mb-1">Jenis Keperluan <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <select v-model="rentalForm.type" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 appearance-none bg-white cursor-pointer" :class="[{'text-gray-400': rentalForm.type === ''}, isBuyMode ? 'focus:ring-red-500' : 'focus:ring-orange-500']">
                  ...
                  <div v-if="rentalForm.type === 'Perusahaan'" class="col-span-1 animate-in slide-in-from-top-2 duration-300">
                    <label class="block text-sm font-normal text-gray-700 mb-1">Nama Perusahaan <span class="text-red-500">*</span></label>
                    <input v-model="rentalForm.companyName" required type="text" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2" :class="isBuyMode ? 'focus:ring-red-500' : 'focus:ring-orange-500'" placeholder="Contoh: PT Sinar Elektro">
                  </div>

                  <div class="col-span-1">
                    <label class="block text-sm font-normal text-gray-700 mb-1">No. WhatsApp <span class="text-red-500">*</span></label>
                    <input v-model="rentalForm.whatsapp" @input="filterPhone" required type="tel" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2" :class="isBuyMode ? 'focus:ring-red-500' : 'focus:ring-orange-500'" placeholder="Contoh: 0812xxxxx">
                  </div>

                  <div v-if="!isBuyMode">
                    <label class="block text-sm font-normal text-gray-700 mb-1">Durasi Sewa <span class="text-red-500">*</span></label>
                    <input v-model="rentalForm.duration" required type="text" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="Contoh: 3 Bulan">
                  </div>
                  
                   <div :class="{'col-span-1': (isBuyMode && (rentalForm.type === '' || rentalForm.type === 'Pribadi')) || (!isBuyMode && rentalForm.type === 'Perusahaan'), 'col-span-2': (isBuyMode && rentalForm.type === 'Perusahaan') || (!isBuyMode && (rentalForm.type === '' || rentalForm.type === 'Pribadi'))}">
                     <label class="block text-sm font-normal text-gray-700 mb-1">Alamat <span class="text-red-500">*</span></label>
                     <input v-model="rentalForm.address" required type="text" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2" :class="isBuyMode ? 'focus:ring-red-500' : 'focus:ring-orange-500'" placeholder="Masukan alamat lengkap">
                  </div>
              </div>

              <div>
                <label class="block text-sm font-normal text-gray-500 mb-1">Detail Kebutuhan <span class="text-red-500">*</span></label>
                <textarea v-model="rentalForm.details" required rows="3" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2" :class="isBuyMode ? 'focus:ring-red-500' : 'focus:ring-orange-500'" :placeholder="isBuyMode ? 'Contoh: Saya butuh 1 unit untuk pabrik di Jakarta...' : 'Contoh: untuk backup kebutuhan kantor, dll...'"></textarea>
              </div>
...
<script>
import { 
  Loader2, Settings, Droplet, Zap, Box, CalendarDays, Headset, 
  XCircle, Lock, ChevronDown, CalendarClock, ShoppingBag, ArrowRight 
...
export default {
  ...
  async mounted() {
    ...
    window.scrollTo(0, 0)
    window.addEventListener("open-cart-modal", this.handleOpenCartFromNavbar);
  },
  beforeUnmount() {
    window.removeEventListener("open-cart-modal", this.handleOpenCartFromNavbar);
  },
  methods: {
    formatRupiah(amount) {
      ...
      try {
        ...
        if (response.data.meta.success) {
          ...
          this.product = {
            ...
            rawPrice: data.sell_price, 
            purchasePrice: this.formatRupiah(data.sell_price),
            category: data.category?.name || null,
            stock: data.stock,
            image: data.image ? `${this.baseURL}/${data.image}` : 'https://images.pexels.com/photos/1797428/pexels-photo-1797428.jpeg?auto=compress&cs=tinysrgb&w=800',
            details: {
              power: { prime: spec.prime_power || '-', standby: spec.standby_power || '-', voltage: spec.voltage || '-', alternator: spec.alternator || '-' },
              engine: { brand: spec.brand || '-', model: spec.model || '-', cylinder: spec.cylinder || '-', pistonDispl: spec.piston_displ || '-', ratedSpeed: spec.rated_speed || '-', boreXStroke: spec.bore_stroke || '-', governor: spec.governor || '-', aspiration: spec.aspiration || '-', oilCapacity: spec.oil_capacity || '-', coolingSystem: spec.cooling_system || '-' },
              fuel: { load100: spec.load_100 || '-', load75: spec.load_75 || '-', load50: spec.load_50 || '-' },
              dimension: { 
                openType: { size: spec.dimension_open || '- x - x - mm', weight: spec.weight_open || '- Kg' },
                silentType: { size: spec.dimension_silent || '- x - x - mm', weight: spec.weight_silent || '- Kg' }
              }
            }
          }
        }
      } catch (error) { this.fetchError = 'Gagal memuat data produk' } finally { this.isLoading = false }
    },
    formatKey(key) {
    ...
    // LOGIKA KERANJANG ASLI (JANGAN DIHAPUS)
    addToCart() {
      const cart = JSON.parse(localStorage.getItem('cart') || '[]');
      const existingItem = cart.find(item => item.uuid === this.product.uuid);
      if (existingItem) { existingItem.quantity += 1; } 
      else { cart.push({ uuid: this.product.uuid, title: this.product.title, image: this.product.image, rawPrice: this.product.rawPrice, quantity: 1 }); }
      localStorage.setItem('cart', JSON.stringify(cart));
      window.dispatchEvent(new CustomEvent('cart-updated', { detail: { productName: this.product.title } }));
    },
    handleOpenCartFromNavbar() {
      this.cartItems = JSON.parse(localStorage.getItem('cart') || '[]');
      this.isBuyMode = true;
      this.isModalOpen = true;
    },

    // LOGIKA FORM & MODAL PINDAHAN DARI FILE (1)
    resetForm() {
      this.rentalForm = {
    ...
    // WHATSAPP TEMPLATE (DITINGKATKAN)
    submitRentalForm() {
      ...
      if (form.type === 'Perusahaan') { message += `Nama Perusahaan: ${form.companyName}\n` }
      message += `No. WhatsApp: ${form.whatsapp}\n`
      message += `Alamat Lokasi: ${form.address}\n`
      message += `Durasi Sewa: ${form.duration}\n\n`
      if (form.details) { message += `DETAIL KEBUTUHAN:\n${form.details}\n\n` }
    ...
    submitBuyForm() {
      ...
      if (form.details) { message += `DETAIL PERMINTAAN:\n${form.details}\n\n` }
...
.slide-in-from-top-2 { animation: slide-down 0.2s ease-out forwards; }
</style>