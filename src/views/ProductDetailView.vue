<template>
  <div class="min-h-screen bg-white text-slate-800">
    ...
    <div v-if="product" class="max-w-[1440px] mx-auto px-4 md:px-8 py-12">
...
export default {
  ...
  data() {
    return {
      ...
      productsDb: [
        { 
          id: 'p1', 
          title: 'Perkins 13 kVA Silent Type', 
          brand: 'PERKINS', 
          model: '403A-15G1',
          ...
          details: {
            power: { prime: '13 kVA', standby: '14 kVA', voltage: '380 Volt', alternator: 'Stamford' },
            engine: { brand: 'PERKINS', model: '403A-15G1', cylinder: '3 L', pistonDispl: '1.496 L', ratedSpeed: '1500 RPM', boreXStroke: '84 x 90 mm', governor: 'Mechanical', aspiration: 'Natural', oilCapacity: '6.0 L', coolingSystem: 'Water Cooled' },
            fuel: { load100: '3.6 Ltr/hour', load75: '2.8 Ltr/hour', load50: '2.0 Ltr/hour' },
            dimension: {
              openType: { size: '1100 x 750 x 1000 mm', weight: '350 Kg' },
              silentType: { size: '1850 x 850 x 1050 mm', weight: '780 Kg' }
            }
          }
        },
        { 
          id: 'p2', 
          title: 'Yanmar 60 kVA Silent', 
          brand: 'YANMAR', 
          model: '4TNV98T',
          purchasePrice: 'Rp 210.000.000',
          image: 'https://images.pexels.com/photos/1267338/pexels-photo-1267338.jpeg?auto=compress&cs=tinysrgb&w=800',
           details: {
            power: { prime: '60 kVA', standby: '66 kVA', voltage: '380 Volt', alternator: 'Stamford' },
            engine: { brand: 'YANMAR', model: '4TNV98T', cylinder: '4 L', pistonDispl: '3.319 L', ratedSpeed: '1500 RPM', boreXStroke: '98 x 110 mm', governor: 'Electronic', aspiration: 'Turbocharged', oilCapacity: '10.5 L', coolingSystem: 'Water Cooled' },
            fuel: { load100: '12.5 Ltr/hour', load75: '9.4 Ltr/hour', load50: '6.5 Ltr/hour' },
            dimension: {
              openType: { size: '2000 x 900 x 1200 mm', weight: '950 Kg' },
              silentType: { size: '2400 x 1000 x 1250 mm', weight: '1400 Kg' }
            }
          }
        },
        {
          id: 'generic',
          title: 'Industrial Genset Unit',
          brand: 'GENERIC',
          model: 'Standard Series',
          purchasePrice: 'Hubungi Kami',
          image: 'https://images.pexels.com/photos/258154/pexels-photo-258154.jpeg?auto=compress&cs=tinysrgb&w=800',
          details: {
            power: { prime: 'Check Spec', standby: '-', voltage: '380 V', alternator: 'Stamford Type' },
            engine: { brand: 'Industrial', model: '-', cylinder: 'In-line', pistonDispl: '-', ratedSpeed: '1500 RPM', boreXStroke: '-', governor: 'Mech/Elec', aspiration: '-', oilCapacity: '-', coolingSystem: 'Water' },
            fuel: { load100: '-', load75: '-', load50: '-' },
            dimension: { openType: { size: '- x - x - mm', weight: '- Kg' }, silentType: { size: '- x - x - mm', weight: '- Kg' } }
          }
        }
      ]
    }
  ...
  methods: {
    loadProduct(id) {
      const found = this.productsDb.find(p => p.id === id);
      if (found) {
        this.product = found;
      } else {
        const generic = JSON.parse(JSON.stringify(this.productsDb.find(p => p.id === 'generic')));
        generic.id = id;
        generic.title = 'Genset ' + id.toUpperCase() + ' Series';
